...
<p align="center">
  <img src="封面.png" alt="Hey-Rice · 小瓜喂饭陪伴 Agent" width="100%">
</p>
...
```text
  ...
  └───────────┘        │      按住说话(PTT)→ 音频流式进豆包 ASR → 松开定稿 → 最终文本→process              │
                       │                                                                                    │
...
```text
浏览器：按住麦克风按钮 → speak_start
  → 按住期间的 16kHz PCM 流式进豆包 ASR（WebSocket /ws/voice）
松开按钮 → speak_end → 定稿
  → 最终文本 → /ws/agent → server.process()（与打字同一套逻辑）
```

**按住说话、松开发送**：不需要唤醒词，也不需要本地唤醒模型——按住麦克风按钮时才录音并上云，
松开即定稿发送。首次按下会请求麦克风权限。
...
| `voice_gateway.py` | 浏览器语音（按键对讲 PTT）、豆包 ASR、agent WebSocket 状态机 |
| `doubao_asr.py` | 豆包流式 ASR 单句会话适配器 |
| `wakeword.py` | sherpa-onnx 本地唤醒检测器（唤醒词方案遗留，PTT 不使用） |
| `agent_ws_client.py` | 最终文本 → 现有 agent 的 WebSocket 适配器 |
| `robot_tts.py` | 把回复文本 POST 给机器人语音服务（`/say`） |
| `static/voice-client.js` · `pcm-worklet.js` | 前端麦克风采集（按住说话）、PCM 重采样 |
| `scripts/setup_kws.py` | 下载唤醒模型（唤醒词方案遗留，PTT 不使用） |
...
- **按住才录音上云**：不按住麦克风时不录音、不上云;松开即结束发送。
